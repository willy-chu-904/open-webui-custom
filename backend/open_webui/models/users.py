...
class UserModel(BaseModel):
    ...
    def validate_role(cls, v: str) -> str: 
        allowed_roles = ["pending", "user", "pro", 
"admin"]

        if v not in allowed_roles: 
            raise ValueError(f"Invalid role: {v}. Must 
...
f"/api/v1/users/{self.id}/profile/image"
...
def is_admin_user(self, user_id: str, db: 
Optional[Session] = None) -> bool:
...
db.query(User).filter_by(id=user_id).first()
            ...
            return user.role == "admin" if user else 
False


    def is_pro_user(self, user_id: str, db: 
Optional[Session] = None) -> bool:
...
db.query(User).filter_by(id=user_id).first()
    ...
    def get_pro_users(self, db: Optional[Session] = 
None) -> list[UserModel]:

        """獲取所有 Pro 用戶""" 
        with get_db_context(db) as db: 
            users = db.query(User).filter(User.role == 
"pro").all()
    return [UserModel.model_validate(user) for 
user in users]
